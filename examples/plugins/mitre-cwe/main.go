...
func main() {
	...
	if v := strings.TrimSpace(os.Getenv("CWE_API_BASE_URL")); v != "" {
		cfg["api_base_url"] = v
	}
	if os.Getenv("CWE_INCLUDE_DEPRECATED") == envTrue {
		cfg["include_deprecated"] = "true"
	}

	status, err := client.RegisterPlugin(ctx, &cwePlugin{}, cartograph.RegisterPluginOptions{
		Config:  cfg,
